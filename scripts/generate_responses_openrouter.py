...
def generate_single_response(question, model_id, api_key, max_retries=MAX_RETRIES):
    """
    Generate a single response using OpenRouter API with retry logic and automatic reconnection.
    ...
    Returns:
        Generated text string on success, None on failure after all retries
    """
    headers = {
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                wait_time = min(retry_delay, MAX_RETRY_DELAY)
                print(f"Request timeout. Reconnecting and waiting {wait_time}s before retry {attempt + 1}/{max_retries}...")
                time.sleep(wait_time)
                retry_delay *= RETRY_BACKOFF_MULTIPLIER
                continue
            else:
                print(f"Request timeout after {max_retries} retries: {e}")
                return None

        except requests.exceptions.ConnectionError as e:
            if attempt < max_retries:
                wait_time = min(retry_delay, MAX_RETRY_DELAY)
                print(f"Connection error. Reconnecting and waiting {wait_time}s before retry {attempt + 1}/{max_retries}...")
                time.sleep(wait_time)
                retry_delay *= RETRY_BACKOFF_MULTIPLIER
                continue
            else:
            ...
            if status_code and status_code in RETRYABLE_STATUS_CODES:
                if attempt < max_retries:
                    ...
                    if retry_after:
                        wait_time = int(retry_after) + 1
                        print(f"HTTP error {status_code}. Waiting {wait_time}s before retry {attempt + 1}/{max_retries}...")
                        time.sleep(wait_time)
                    else:
                        wait_time = min(retry_delay, MAX_RETRY_DELAY)
                        print(f"HTTP error {status_code}. Waiting {wait_time}s before retry {attempt + 1}/{max_retries}...")
                    ...
                    print(f"HTTP error {status_code} after {max_retries} retries: {e}")
                    return None
            else:
            ...
            if attempt < max_retries:
                wait_time = min(retry_delay, MAX_RETRY_DELAY)
                print(f"Unexpected error. Waiting {wait_time}s before retry {attempt + 1}/{max_retries}: {e}")
                time.sleep(wait_time)
                retry_delay *= RETRY_BACKOFF_MULTIPLIER
                continue
            else:
                print(f"Unexpected error after {max_retries} retries: {e}")
                return None
...
def generate_responses_for_model(model_name, model_id, questions, api_key, output_file, num_generations_per_question, max_retries=MAX_RETRIES):
    """
    ...
    with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
        # Submit all tasks
        future_to_task = {
            executor.submit(generate_single_response, question, model_id, api_key, max_retries): (question, gen_idx)
...
def main():
    ...
    for model_name, model_id in models_to_generate.items():
        ...
        completed_questions, total_responses, fail_count = generate_responses_for_model(
            model_name, model_id, questions, api_key, output_file, args.num_generations, args.max_retries
        )
