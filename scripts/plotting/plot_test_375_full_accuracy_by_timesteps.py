...
def get_test_375_full_accuracies(classifier_dir, results_dir, model_type):
    dir_patterns = ['test_375_full', 'full_375_test']

    qwen_acc = None
    llama_acc = None

    for dir_pattern in dir_patterns:
        file_path = os.path.join(results_dir, classifier_dir, dir_pattern, f'evaluation_results_{model_type}_full_375_test.json')
...
def get_bert_test_375_full_accuracies(classifier_dir, results_dir):
    dir_patterns = ['test_375_full', 'full_375_test']
    ...
    for dir_pattern in dir_patterns:
        file_path = os.path.join(results_dir, classifier_dir, dir_pattern, 'evaluation_results_bert_full_375_test.json')
        if os.path.exists(file_path):
            qwen_acc = load_per_model(file_path, 'qwen')
            llama_acc = load_per_model(file_path, 'llama')
            if qwen_acc is not None and llama_acc is not None:
...
def main():
    ...
    for step in timesteps:
        ...
        qwen_acc, llama_acc, overall_acc = get_bert_test_375_full_accuracies(classifier, results_dir)
    ...
    for step in timesteps:
        ...
        qwen_acc, llama_acc, overall_acc = get_test_375_full_accuracies(classifier, results_dir, 'llm')
    ...
    csv_file = 'classifiers/results/.data/test_375_full_accuracy_data.csv'
    ...
    fig.suptitle('Classifier Accuracy on Responses by Unseen Fine-tuned Variants',
                 fontsize=15, fontweight='bold', y=1.02)

    plt.tight_layout()

    output_file = 'classifiers/results/.plots/test_375_full_accuracy_by_timesteps.png'
