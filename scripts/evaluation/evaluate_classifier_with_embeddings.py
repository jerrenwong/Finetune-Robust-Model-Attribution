...
class EmbeddingDataset(Dataset):
    """Dataset that loads pre-computed CLS embeddings from disk."""
    def __init__(self, embeddings_path):
        ...
        self.source_models = data['source_models'].tolist()
        self.use_cls = bool(data['use_cls'])
    ...
    def __getitem__(self, idx):
        return self.embeddings[idx], self.labels[idx], self.source_models[idx]
...
# ===========================
# CLASSIFIER HEAD ONLY (for pre-computed embeddings)
# ===========================

class ClassifierHeadOnly(nn.Module):
    """Classifier head that takes pre-computed embeddings as input."""
    def __init__(self, hidden_size, head_type="bert"):
        super().__init__()

        if head_type == "bert":
            # BERT classifier head architecture
            self.classifier = nn.Sequential(
            ...
            # LLM classifier head architecture
            self.classifier = nn.Sequential(
                nn.Linear(hidden_size, 512),
                nn.ReLU(),
                nn.Dropout(0.1),
...
def main():
    ...
    parser.add_argument("--use_embeddings", action="store_true", help="Use pre-computed CLS embeddings if available (much faster)")
    parser.add_argument("--embeddings_file", type=str, default=None, help="Path to pre-computed embeddings file (auto-detects if not specified)")
    ...
    if args.use_embeddings:
        if args.embeddings_file:
            ...
            test_file_basename = os.path.basename(args.test_file)
            if "train" in test_file_basename:
                candidate_name = "train_cls_embeddings.npz"
            elif "final" in test_file_basename or "full" in test_file_basename:
                candidate_name = "final_test_cls_embeddings.npz"
            else:
                candidate_name = "test_cls_embeddings.npz"

            embeddings_path = os.path.join(args.checkpoint_dir, candidate_name)

            if not os.path.exists(embeddings_path):
                print(f"Warning: Pre-computed embeddings not found at {embeddings_path}")
                print("Falling back to full model evaluation.")
                embeddings_path = None
            else:
                print(f"Auto-detected embeddings file: {embeddings_path}")

    use_embeddings = embeddings_path is not None and os.path.exists(embeddings_path)

    if use_embeddings:
        print("=" * 60)
        print("USING PRE-COMPUTED EMBEDDINGS - Much faster evaluation!")
        print("=" * 60)

        # Load embedding dataset
        dataset = EmbeddingDataset(embeddings_path)

        # Create a simple dataloader for embeddings
        def collate_embeddings(batch):
            embeddings = torch.stack([item[0] for item in batch])
            labels = torch.stack([item[1] for item in batch])
            source_models = [item[2] for item in batch]
            return embeddings, labels, source_models

        loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False, collate_fn=collate_embeddings)

        # Load only the classifier head
        print(f"Loading classifier head from {args.checkpoint_dir}")
        model_to_eval = ClassifierHeadOnly(dataset.hidden_size, head_type=args.model_type).to(device)

        if args.model_type == "llm":
            checkpoint_path = os.path.join(args.checkpoint_dir, "classifier_full.pt")
            if os.path.exists(checkpoint_path):
                print(f"Loading weights from {checkpoint_path}")
                state_dict = torch.load(checkpoint_path, map_location=device)
                # Extract only classifier head weights
                classifier_keys = {k.replace("classifier.", ""): v for k, v in state_dict.items() if k.startswith("classifier.")}
                model_to_eval.classifier.load_state_dict(classifier_keys)
            else:
        ...
        print("Running evaluation with pre-computed embeddings...")
        with torch.no_grad():
            for embeddings, labels, source_models in tqdm(loader):
                embeddings = embeddings.to(device)
                labels = labels.to(device)

                logits = model_to_eval(embeddings)
        ...
        print("=" * 60)
        print("LOADING FULL MODEL - This may take a while...")
        print("=" * 60)

        # Load Model (original logic)
        if args.model_type == "llm":
