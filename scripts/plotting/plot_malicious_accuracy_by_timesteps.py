...
def get_malicious_accuracies(classifier_dir, results_dir):
    dir_patterns = {
        'uncensored_qwen': ['uncensored_qwen_malicious_1k', 'uncensored_qwen_malicious'],
        'uncensored_llama': ['uncensored_llama_malicious_1k', 'uncensored_llama_malicious']
    }

    uncensored_qwen_qwen = None
    for dir_pattern in dir_patterns['uncensored_qwen']:
        file_path = os.path.join(results_dir, classifier_dir, dir_pattern, 'evaluation_results_llm_malicious_1k.json')
    ...
    qwen_response_acc = uncensored_qwen_qwen * 100
    llama_response_acc = uncensored_llama_llama * 100
    overall_acc = (qwen_response_acc + llama_response_acc) / 2

    return qwen_response_acc, llama_response_acc, overall_acc

def get_bert_malicious_accuracies(classifier_dir, results_dir):
    dir_patterns = {
        'uncensored_qwen': ['malicious_1k_uncensored_qwen', 'uncensored_qwen_malicious'],
        'uncensored_llama': ['malicious_1k_uncensored_llama', 'uncensored_llama_malicious']
    }

    uncensored_qwen_qwen = None
    for dir_pattern in dir_patterns['uncensored_qwen']:
        file_path = os.path.join(results_dir, classifier_dir, dir_pattern, 'evaluation_results_bert_malicious_1k.json')
        if os.path.exists(file_path):
            uncensored_qwen_qwen = load_per_model(file_path, 'qwen')
            if uncensored_qwen_qwen is not None:
                break
...
def main():
    ...
    for step in timesteps:
        ...
        qwen_acc, llama_acc, overall_acc = get_bert_malicious_accuracies(classifier, results_dir)
    ...
    for step in timesteps:
        ...
        qwen_acc, llama_acc, overall_acc = get_malicious_accuracies(classifier, results_dir)
    ...
    csv_file = 'classifiers/results/.data/malicious_accuracy_data.csv'
    ...
    ax1.set_title('BERT Classifier', fontsize=14, fontweight='bold', pad=15)
    ...
    ax2.set_title('Qwen LLM Classifier', fontsize=14, fontweight='bold', pad=15)
    ...
    ax3.set_title('Llama LLM Classifier', fontsize=14, fontweight='bold', pad=15)
    ...
    fig.suptitle('Classifier Accuracy on Uncensored Model\'s Response towards Malicious Questions',
                 fontsize=15, fontweight='bold', y=1.02)

    plt.tight_layout()

    output_file = 'classifiers/results/.plots/malicious_accuracy_by_timesteps.png'
